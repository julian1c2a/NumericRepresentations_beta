...
namespace NumRepr {
  ...
  public:
    ...
    /// ADELANTADO AQUÍ POR NECESIDAD
    consteval inline dig_t() noexcept : m_d(0u) {}

  private:
    /// FUNCION NORMALIZA UN ENTERO A UN DIGITO MODULAR
    /// ES UN MÉTODO ESTÁTICO FUNDAMENTAL A LA CLASE TEMPLATE DIG_T
    /// EN DIG_T SE GUARDA UN NÚMERO MÓDULO LA BASE B,
    /// POR LO QUE LA CANTIDAD DE VALORES QUE PUEDE TOMAR UN DÍGITO
    /// ES FINITA Y VA DE 0 A B-1
    /// ESTA FUNCIÓN NORMALIZA DESDE VALORES NEGATIVOS Y POSITIVOS
  ...
  public:
    /// CONSTRUCTORES
    /// CONSTRUCTOR POR DEFECTO (ANTERIORMENTE ADELANTADO)
        /// consteval inline dig_t() noexcept : m_d(0u) {}
    /// CONSTRUCTOR DADO UN ENTERO
    template <type_traits::integral_c Int_t>
    ...
    /// CONSTRUCTOR DADO UNA CADENA DE CARACTERES (EXPLÍCITOS)
    explicit dig_t(const std::string &str);
    explicit constexpr dig_t(const char *str);

    /// FUNCIONES HACEN PARSING DESDE UNA CADENA DE CARACTERES (STATIC)
    ...
    /// OPERADOR DE ASIGNACIÓN DESDE UN ENTERO
    template <type_traits::integral_c Int_t>
    ...
    /// OPERADOR DE ASIGNACIÓN DESDE UNA REFERENCIA CONSTANTE
    constexpr dig_t &operator=(const dig_t &) noexcept = default;
    /// OPERADOR DE ASIGNACIÓN POR MOVIMIENTO
    constexpr dig_t &operator=(dig_t &&) noexcept = default;

    /// FUNCIÓN IS_UNIT DESDE EL PUNTO DE VISTA DE UN 
    /// ANILLO CONMUTATIVO CON UNIDAD
    /// VERDADERO SI EXISTE EL ELEMENTO INVERSO MULTIPLICATIVO
