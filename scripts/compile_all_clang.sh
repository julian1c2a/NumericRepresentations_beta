#!/bin/bash
# Script para compilar con Clang - todas las suites de testing
# Requiere: LLVM/Clang instalado

echo "=== CONFIGURACIÓN CLANG PARA NumericRepresentations ==="
echo "Compilador: $(clang --version | head -1)"
...
TESTS_DIR="../tests"
OUTPUT_DIR="../tests" 
CLANG_FLAGS="-std=c++23 -Wall -Wextra -O2"
...
compile_and_run() {
    ...
    if clang++ $CLANG_FLAGS -I$INCLUDE_DIR $TESTS_DIR/$test_file -o $OUTPUT_DIR/${output_name}_clang.exe; then
        echo "✅ Compilación exitosa"
        echo
        echo "=== Ejecutando $description ==="
        if $OUTPUT_DIR/${output_name}_clang.exe; then
